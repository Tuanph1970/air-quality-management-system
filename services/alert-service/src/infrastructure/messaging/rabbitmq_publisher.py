"""RabbitMQ event publisher adapter for the alert service.
...
from shared.messaging.config import ALERT_EXCHANGE
...
class RabbitMQEventPublisher:
    ...
    ``RabbitMQPublisher.publish(event, exchange)`` adding the alert
    ...
    async def connect(self) -> None:
        ...
        logger.info("Alert event publisher connected")

    async def close(self) -> None:
        """Gracefully close the RabbitMQ connection."""
        await self._publisher.close()
        logger.info("Alert event publisher closed")

    # ------------------------------------------------------------------
    # EventPublisher protocol
    # ------------------------------------------------------------------
    async def publish(self, event: DomainEvent) -> None:
        """Publish a domain event to the alert exchange.
