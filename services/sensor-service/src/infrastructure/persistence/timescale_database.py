"""Async SQLAlchemy engine and session configuration for TimescaleDB.

Provides the ``AsyncEngine``, ``async_session_maker``, and a FastAPI-
compatible ``get_db()`` dependency that yields a scoped ``AsyncSession``.

All tunables are read from ``src.config.settings`` so there is a single
...
class Base(DeclarativeBase):
    """SQLAlchemy declarative base for sensor-service models."""
